...
export default function Sidebar({
  currentPage, onNavigate, collapsed, mobileOpen = false, onCloseMobile,
}: SidebarProps) {
  ...
  return (
    <>
      {mobileOpen && (
        <button type="button" aria-label="Close navigation" onClick={onCloseMobile}
      ...
      <aside className={`${mobileOpen ? 'fixed inset-y-0 left-0 z-50 flex w-72' : 'hidden md:flex'} ${collapsed ? 'md:w-16' : 'md:w-60'} flex-shrink-0 abos-bg border-r border-white/[0.06] flex-col transition-all duration-300`}>
        <div className="h-16 flex items-center justify-between px-3 border-b border-white/[0.06]">
          ...
          <button type="button" onClick={onCloseMobile} aria-label="Close navigation" className="md:hidden p-2 text-slate-400 hover:text-white rounded-lg hover:bg-white/[0.05]">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { onNavigate(item.id); onCloseMobile?.(); }} title={collapsed ? item.label : undefined}
